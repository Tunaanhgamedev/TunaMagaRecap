export const MieuTruyenAdapter = {
  name: 'MieuTruyen Adapter',
  domains: [
    'mieutruyen.com', 'mieutruyen.net',
  ],
  ...
  async getMangaInfo(url) {
    ...
    let title = 'Truyện Tranh MieuTruyen';
    let chapterNumber = 1;
    ...
    if (raw) {
      title = raw
        .replace(/[-|].*Miêu Truyện.*/gi, '')
        .replace(/[-|].*MieuTruyen.*/gi, '')
    ...
    return { title, chapterNumber, sourceName: 'MieuTruyen', sourceUrl: url, html };
  },
    ...
    while ((m = regex.exec(html)) !== null) {
      ...
      if (
        ...
        (src.includes('/uploads/') || src.includes('/chapter/') || src.includes('/manga/') ||
         src.includes('/comic/') || src.includes('.jpg') || src.includes('.png') || src.includes('.webp')) &&
        !src.includes('logo') && !src.includes('favicon') && !src.includes('banner') &&
        !src.includes('avatar') && !src.includes('ads') && !src.includes('icon') &&
        !src.includes('thumb') && !src.includes('150x') && !src.includes('user/')
      ) {
